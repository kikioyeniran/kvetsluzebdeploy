            ...
            if (empty(schedule)) {
              ...
              CleaningSchedule.countDocuments(query2, function(err, c) {
                ...
                for (var i = 0; i < count; i++) {
                  ...
                  var currentCleanDate =
                    tempSchedule.currentClean[0].currentCleanDate;
                  var currentCleanDate = new Date(currentCleanDate);
                  var currentCleanDate = date.format(
                    currentCleanDate,
                    'ddd, MMM DD YYYY'
                  );
                  var nextCleanDate =
                    tempSchedule.currentClean[0].nextCleanDate;
                  var nextCleanDate = new Date(nextCleanDate);
                  var nextCleanDate = date.format(
                    nextCleanDate,
                    'ddd, MMM DD YYYY'
                  );
                ...
                console.log(newArray[0].clientDetails.fullName);
                res.render('cleaner/cleaner_calendar', {
                  cleaner: cleaner,
